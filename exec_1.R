  ...
  for (j in dags) {
    ...
    for (i in 1:nrow(metadata_tmp)){
      ...
      if(!is.na(form_complete)){
        if(is.na(expr_str))
          ps_tmp <- subset(redcap_data_j, get(form_complete) == 1)
        else
          ps_tmp <- subset(redcap_data_j, get(form_complete) == 1 & eval(parse(text = expr_str)))
      } else {
        ...
        if(is.na(expr_str))
          ps_tmp <- subset(redcap_data_j, redcap_repeat_instrument == event)
        else
          ps_tmp <- subset(redcap_data_j, redcap_repeat_instrument == event & eval(parse(text = expr_str)))
      }
    ...
    # add worksheet
    addWorksheet(wb, paste0(toupper(j),"-Miss&Unk"))
    
    # write data on worksheet
    # Missing & Unknown
    writeData(wb,sheet = paste0(toupper(j),"-Miss&Unk"), check_summary, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
    setColWidths(wb,sheet = paste0(toupper(j),"-Miss&Unk"),cols = 1:ncol(check_summary), widths = "auto")
  }
} else {
  # create a new dataframe to count % of missing and % of missing+unknown
  check_summary <- data.frame("var" = character(0), "var_lab" = character(0), "form" = character(0), "den" = integer(0), "perc_miss" = double(0), "perc_miss_unk" = double(0))
