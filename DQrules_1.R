...
# Exclude dag from analyses if AIOCC 
if(properties$id_centro == id_aiocc) {
  excluded_aiocc <- grep(paste0(id_aiocc, "-"), exclude_from_analyses, value = TRUE) %>% 
    gsub(paste0(id_aiocc, "-"), "", .)
  
  pts_id <- pts_id %>%
    filter(!(redcap_data_access_group %in% excluded_aiocc))
}

# Check if there is at least 1 patient: if yes continue, if no create the needed files and then stop the execution
if(!nrow(pts_id)) {
  ##### PS-CTRL #####
  # Save the current completeness status (Complete, Incomplete) 
  ps_ctrl <- setNames(data.frame(matrix(ncol = 22, nrow = 0)), 
                      c("a01_id", "redcap_event_name", "redcap_repeat_instrument", "redcap_repeat_instance", "a07_complete", "b24_complete","c28_complete",
                        "d34_complete", "e57_complete", "f107_complete", grep("g[[:digit:]]+_119_complete", metadata$field_name, value = TRUE), "h06_complete", "i07_complete")) # write csv with ps_ctrl
  ...
  writeData(wb,sheet = "Checks", "No records exist yet")
  ...
  file.rename(from = paste0("/var/lib/docker/volumes/vantage6-starter_head_and_neck-user-vol/_data/", file_name), to = paste0("/var/lib/docker/volumes/vantage6-starter_head_and_neck-user-vol/_data/", run_id, "-QC-individual.xlsx"))
  
  ##### QC SUMMARY 
  wb <- createWorkbook()
  addWorksheet(wb, "Missing & Unknown")
  writeData(wb,sheet = "Missing & Unknown", "No records exist yet")
...
### Rule #1.2 --------------------------------------------------------------------
# If cancer phase managed by the hospital is treatment of primary tumor (a06_phase___2 = 1), at least one treatment must be done at the hospital
data_rule1_2 <- ddply(subset(redcap_data, a07_complete == 1 & a06_phase___2 == 1 & f107_complete == 2), c("a01_id"), summarize,
                      category = "ERROR",
                      description = "Cancer phase managed by the hospital is 'Treatment of primary tumor' but no treatments are performed at the hospital",
                      form1 = "Preliminary",
                      event1 = "NON REPEATABLE",
                      instance1 = redcap_repeat_instance,
                      var1 = gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "a06_phase")])),
                      status = ifelse(f01_surgery == 1 | f29_1_systrt == 1 | f29_2_systrt == 1 | f29_3_systrt == 1 | f53_radio == 1,'PASS','FAIL'))

### Rule #1.3 --------------------------------------------------------------------
# If a06_phase = 3 OR 4 (treatment recurrence), at least one 7s must be gx_01_progrel = 2 (Please specify = Recurrence) 
data_rule1_3 <- ddply(subset(redcap_data, a07_complete == 1 & (a06_phase___3 == 1 | a06_phase___4 == 1)), c("a01_id"), summarize,
                      category = "ERROR",
                      description = "Cancer phase managed by the hospital is 'Treatment of recurrence' but no recurrence is specified",
                      form1 = "Preliminary",
                      event1 = "NON REPEATABLE",
                      instance1 = redcap_repeat_instance,
                      var1 = gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "a06_phase")])),
                      status = ifelse(any(mget(grep("g[[:digit:]]_119_complete", colnames(redcap_data), value = TRUE), inherits = TRUE) == 1) & any(mget(grep("g[[:digit:]]_01_progrel", colnames(redcap_data), value = TRUE), inherits = TRUE) == 2) , 'PASS','FAIL'))

### Rule #1.4 --------------------------------------------------------------------
# If a06_phase = 5 (M+), at least one between  f20_surgm = 1 (Surgery on M = yes) OR f85_radiosite = 5 (Treatment Sites (multiple selection) = Distant Metastasis) OR gx_06_meta = 1 (Metastatic = yes) must be true
data_rule1_4 <- ddply(subset(redcap_data, a07_complete == 1 & a06_phase___5 == 1), c("a01_id"), summarize,
                      category = "ERROR",
                      description = "Cancer phase managed by the hospital is 'Treatment of M+' but no treatment on metastasis is specified",
                      form1 = "Preliminary",
                      event1 = "NON REPEATABLE",
                      instance1 = redcap_repeat_instance,
                      var1 = gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "a06_phase")])),
                      status = ifelse((f107_complete == 1 | any(mget(grep("g[[:digit:]]_119_complete", colnames(redcap_data), value = TRUE), inherits = TRUE) == 1)) & (f20_surgm == 1 | f85_radiosite___5 == 1 | any(mget(grep("g[[:digit:]]_06_meta", colnames(redcap_data), value = TRUE), inherits = TRUE) == 1)) , 'PASS','FAIL'))
...
data_rule2 <- ddply(subset(redcap_data, b24_complete == 1 & a07_complete == 1 &
                    ...
                    value2 = ifelse(!is.na(a05_date_unk) & format(a05_date, "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", a05_date)),length=2,by="months")[2]-1, a05_date),
...
data_rule20_2 <- ddply(subset(redcap_data, d34_complete == 1 & !(is.na(d21_ebv)) & (d11_siterare == 1 | d11_sitecomrar == 1) & d06_histosubg_sq != 7), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Plasmatic EBV DNA is compiled but Histology subgroup Squamous is not Lymphoepithelial carcinoma",
                       form1 = "Cancer under study",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "d21_ebv")],
                       value1 = d21_ebv,
                       form2 = "Cancer under study",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "d11_siterare")],
                       value2 = d11_siterare,
                       form3 = "Cancer under study",
                       event3 = "NON REPEATABLE",
                       instance3 = redcap_repeat_instance,
                       var3 = metadata$field_label[which(metadata$field_name == "d06_histosubg_sq")],
                       value3 = d06_histosubg_sq,
                       status = 'FAIL')
...
### Rule #31 --------------------------------------------------------------------
# if surgery = yes, pathological stage = applicable, pathological stage (e54_pstage) must be filled in

data_rule31 <- ddply(subset(redcap_data, e57_complete == 1 & f107_complete == 1 & f01_surgery %in% c(1,2) & e37_ps_avail == 1), c("a01_id"), summarize,
                     category = "ERROR",
                     description = "SURGERY PERFORMED: Pathological staging is applicable but is not specified",
...
### Rule #32 --------------------------------------------------------------------
# if surgery = yes at least Date of surgery of cancer under study (f02_datesurg) OR Date of neck surgery (f12_datenecksurg) OR Date of surgery on M (f22_datesurgm) must be compiled

data_rule32 <- ddply(subset(redcap_data, f107_complete == 1 & f01_surgery %in% c(1,2)), c("a01_id"), summarize,
                     category = "ERROR",
                     description = "Surgery is performed, but no date is specified.",
                     form1 = "Cancer Under Study Treatment",
                     event1 = "NON REPEATABLE",
                     instance1 = redcap_repeat_instance,
                     var1 = metadata$field_label[which(metadata$field_name == "f01_surgery")],
                     value1 = f01_surgery,
                     status = ifelse(!all(is.na(c(f02_datesurg,f12_datenecksurg,f22_datesurgm))), 'PASS','FAIL'))
...
data_rule33 <- ddply(subset(redcap_data, f107_complete == 1 & d34_complete == 1 & !(is.na(f02_datesurg)) & !(is.na(d01_diagdate))), c("a01_id"), summarize,
                     ...
                     event1 = "NON REPEATABLE",
                     instance1 = redcap_repeat_instance,
                     var1 = gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "f02_datesurg")])),
                     value1 = ifelse(!is.na(f02_datesurg_unk) & format(f02_datesurg, "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", f02_datesurg)),length=2,by="months")[2]-1, f02_datesurg),
                     form2 = "Cancer under study",
                     event2 = "NON REPEATABLE",
                     instance2 = redcap_repeat_instance,
                     var2 = metadata$field_label[which(metadata$field_name == "d01_diagdate")],
                     value2 = ifelse(!is.na(d01_diagdate_unk) & format(d01_diagdate, "%d") == "15", as.Date(sub("\\d{2}$", "01", d01_diagdate)), d01_diagdate),
...
### Rule #34.2 --------------------------------------------------------------------
# If Date of surgery of cancer under study (f02_datesurg) is compiled, pT (e37_pt) must be filled in

data_rule34_2 <- ddply(subset(redcap_data, e57_complete == 1 & f107_complete == 1 & !is.na(f02_datesurg) & is.na(e37_pt)), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Date of surgery is specified but pT is missing",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "f02_datesurg")])),
                       ...
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "e37_pt")],
                       value2 = e37_pt,
                       status = 'FAIL')

### Rule #34.3 --------------------------------------------------------------------
# If Neck surgery (f11_necksurg) = yes [1], Neck dissection (e44_neckdiss) must be yes [1]

data_rule34_3 <- ddply(subset(redcap_data, e57_complete == 1 & f107_complete == 1 & f11_necksurg == 1 & !is.na(e44_neckdiss) & e44_neckdiss != 1), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Neck surgery is performed but neck dissection is not equal to 'Yes'",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f11_necksurg")],
                       value1 = f11_necksurg,
                       form2 = "Staging procedures and stage",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "e44_neckdiss")],
                       value2 = e44_neckdiss,
                       status = 'FAIL')
...
data_rule35 <- ddply(subset(redcap_data, f107_complete == 1 & d34_complete == 1 & !(is.na(f12_datenecksurg)) & !(is.na(d01_diagdate))), c("a01_id"), summarize,
                     ...
                     value1 = ifelse(!is.na(f12_datenecksurg_unk) & format(f12_datenecksurg, "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", f12_datenecksurg)),length=2,by="months")[2]-1, f12_datenecksurg),
                     form2 = "Cancer under study",
                     event2 = "NON REPEATABLE",
                     instance2 = redcap_repeat_instance,
                     var2 = metadata$field_label[which(metadata$field_name == "d01_diagdate")],
...
### Rule #36.2 --------------------------------------------------------------------
# If Neck dissection (f14_neckdissection) = yes [1], Neck dissection (e44_neckdiss) = yes [1] 

data_rule36_2 <- ddply(subset(redcap_data, e57_complete == 1 & f107_complete == 1 & f14_neckdissection == 1 & !is.na(e44_neckdiss) & e44_neckdiss != 1), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Neck dissection is 'Yes' but neck dissection (Staging) is not equal to 'Yes'",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f14_neckdissection")],
                       value1 = f14_neckdissection,
                       form2 = "Staging procedures and stage",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "e44_neckdiss")],
...
### Rule #36.3 --------------------------------------------------------------------
# If Laterality of the dissection (f15_laterality) is ipsilateral OR controlateral [1,3], Neck dissection side = right OR left [1,2]

data_rule36_3 <- ddply(subset(redcap_data, e57_complete == 1 & f107_complete == 1 & f15_laterality %in% c(1,3) & !is.na(e44_neckdiss_side)), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Laterality of dissection is ipsilateral or controlateral but neck dissection side is not right or left",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f15_laterality")],
                       value1 = f15_laterality,
                       form2 = "Staging procedures and stage",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "e44_neckdiss_side")],
                       value2 = e44_neckdiss_side,
                       status = ifelse(value2 %in% c(1,2), 'PASS', 'FAIL'))

### Rule #36.4 --------------------------------------------------------------------
# If Laterality of the dissection (f15_laterality) is bilateral, Neck dissection side = bilateral [3]

data_rule36_4 <- ddply(subset(redcap_data, e57_complete == 1 & f107_complete == 1 & f15_laterality == 2 & !is.na(e44_neckdiss_side)), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Laterality of dissection is bilateral but neck dissection side is not bilateral",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f15_laterality")],
                       ...
                       var2 = metadata$field_label[which(metadata$field_name == "e44_neckdiss_side")],
                       value2 = e44_neckdiss_side,
                       status = ifelse(value2 == 3, 'PASS', 'FAIL'))

### Rule #36.5 --------------------------------------------------------------------
# If Surgical procedures: right neck (f16_surgproc_rightneck) is filled in AND f16_surgproc_rightneck =! 0, Number of lymph nodes removed, right side (e45_numlympr) must be compiled 

data_rule36_5 <- ddply(subset(redcap_data, e57_complete == 1 & f107_complete == 1 & !is.na(f16_surgproc_rightneck) & !(f16_surgproc_rightneck %in% c(999,0)) & is.na(e45_numlympr)), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Surgical procedure: right neck is performed but number of lymph nodes removed on the right side is not specified",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f16_surgproc_rightneck")],
                       value1 = f16_surgproc_rightneck,
                       form2 = "Staging procedures and stage",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "e45_numlympr")],
...
### Rule #36.6 --------------------------------------------------------------------
# If Surgical procedure: left neck (f18_surgproc_leftneck) is filled in AND f18_surgproc_leftneck != 0, Number of lymph nodes removed, left side (e48_numlympl) must be compiled 

data_rule36_6 <- ddply(subset(redcap_data, e57_complete == 1 & f107_complete == 1 & !is.na(f18_surgproc_leftneck) & !(f18_surgproc_leftneck %in% c(999,0)) & is.na(e48_numlympl)), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Surgical procedure: left neck is performed but number of lymph nodes removed on the left side is not specified",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f18_surgproc_leftneck")],
                       value1 = f18_surgproc_leftneck,
                       form2 = "Staging procedures and stage",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "e48_numlympl")],
...
### Rule #39 --------------------------------------------------------------------
# Site of surgery on metastasis is not consistent with M site
...
data_rule40 <- ddply(subset(redcap_data, f107_complete == 1 & d34_complete == 1 & !(is.na(f22_datesurgm)) & !(is.na(d01_diagdate))), c("a01_id"), summarize,
                     ...
                     value1 = ifelse(!is.na(f22_datesurgm_unk) & format(f22_datesurgm, "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", f22_datesurgm)),length=2,by="months")[2]-1, f22_datesurgm),
                     form2 = "Cancer under study",
                     event2 = "NON REPEATABLE",
                     instance2 = redcap_repeat_instance,
                     var2 = metadata$field_label[which(metadata$field_name == "d01_diagdate")],
...
data_rule42 <- ddply(subset(redcap_data, f107_complete == 1 & !(is.na(f26_dateunplsurg)) & !(is.na(f02_datesurg)) & f26_dateunplsurg > f02_datesurg), c("a01_id"), summarize,
                     ...
                     value1 = ifelse(!is.na(f26_dateunplsurg_unk) & format(f26_dateunplsurg, "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", f26_dateunplsurg)),length=2,by="months")[2]-1, f26_dateunplsurg),
                     form2 = "Cancer Under Study Treatment",
                     event2 = "NON REPEATABLE",
                     instance2 = redcap_repeat_instance,
                     var2 = gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "f02_datesurg")])),
                     value2 = ifelse(!is.na(f02_datesurg_unk) & format(f02_datesurg, "%d") == "15", as.Date(sub("\\d{2}$", "01", f02_datesurg)), f02_datesurg),
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & d34_complete == 1 & !(is.na(get(metadata_tmp$field_name[i]))) & !(is.na(d01_diagdate))), c("a01_id"), summarize,
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("f33_", i, "_startdate_syst_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Cancer under study",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "d01_diagdate")],
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & get(paste0("f32_", i, "_systsetting")) == 1 & !(is.na(get(metadata_tmp$field_name[i]))) & (!(is.na(f02_datesurg)) | !(is.na(f80_radiostartdate)))), c("a01_id"), summarize,
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("f33_", i, "_startdate_syst_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))), get(metadata_tmp$field_name[i])),
                    form2 = "Cancer Under Study Treatment",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = ifelse(is.na(f02_datesurg), metadata$field_label[which(metadata$field_name == "f80_radiostartdate")], gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "f02_datesurg")]))),
...
### Rule #48 --------------------------------------------------------------------
# if adjuvant, date must be posterior to surgery or radiotherapy
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & get(paste0("f32_", i, "_systsetting")) == 3 & !(is.na(get(metadata_tmp$field_name[i]))) & (!(is.na(f02_datesurg)) | !(is.na(f81_radioendate)))), c("a01_id"), summarize,
                    ...
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = ifelse(is.na(f02_datesurg), metadata$field_label[which(metadata$field_name == "f81_radioendate")], gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "f02_datesurg")]))),
...
### Rule #49 --------------------------------------------------------------------
# if concomitant, date should be within the start and end date of radiotherapy (chemo must start 7 gg prior start radio)
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & get(paste0("f32_", i, "_systsetting")) == 2 & !(is.na(get(metadata_tmp$field_name[i]))) & f55_radiosett %in% c(2,4,6) & !is.na(f80_radiostartdate)), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Concomitant systemic treatment starts more than 7 days before radiotherapy",
                    form1 = "Cancer Under Study Treatment",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = get(metadata_tmp$field_name[i]),
                    value1_up = ifelse(!is.na(get(paste0("f33_", i, "_startdate_syst_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Cancer Under Study Treatment",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "f80_radiostartdate")],
                    value2 = ifelse(!is.na(f80_radiostartdate_unk) & format(f80_radiostartdate, "%d") == "15", as.Date(sub("\\d{2}$", "01", f80_radiostartdate)), f80_radiostartdate),
                    status = ifelse(value1_up >= ifelse(get(paste0("f30_", i, "_combofsystrt")) == 1, value2 - 7, value2),'PASS','FAIL'))
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & !(is.na(get(paste0("f33_", i,"_startdate_syst")))) & !(is.na(get(metadata_tmp$field_name[i])))), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "End date of systemic treatment is prior to start date of systemic treatment",
                    form1 = "Cancer Under Study Treatment",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("f34_", i, "_enddate_syst_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Cancer Under Study Treatment",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == paste0("f33_", i,"_startdate_syst"))],
                    value2 = ifelse(!is.na(get(paste0("f33_", i,"_startdate_syst_unk"))) & format(get(paste0("f33_", i,"_startdate_syst")), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(paste0("f33_", i,"_startdate_syst")))), get(paste0("f33_", i,"_startdate_syst"))),
                    status = ifelse(value1 >= value2, 'PASS','FAIL'))
...
### Rule #53.2 --------------------------------------------------------------------
# if regimen [f36_1_regimen e 2 e 3]= cetuximab (29) and setting alone [f32_1_systsetting= 4], systemic treatment must be target +/- chemo [f30_1_combofsystrt= 3]
data_rule53_2 <- data.frame()

var_tmp <- colnames(redcap_data)[grepl("f36_[[:digit:]]_regimen", colnames(redcap_data))]
metadata_tmp <- subset(metadata, field_name %in% var_tmp)

for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & get(metadata_tmp$field_name[i]) == 29 & get(paste0("f32_", i, "_systsetting")) == 4 & !(is.na(get(paste0("f30_", i, "_combofsystrt"))))), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Regimen is cetuximab, setting is systemic treatment alone but systemic treatment is not Target therapy +/- Chemotherapy",
                    form1 = "Cancer Under Study Treatment",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    ...
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == paste0("f32_", i, "_systsetting"))],
                    value2 = get(paste0("f32_", i, "_systsetting")),
                    form3 = "Cancer Under Study Treatment",
                    event3 = "NON REPEATABLE",
                    instance3 = redcap_repeat_instance,
                    var3 = metadata$field_label[which(metadata$field_name == paste0("f30_", i, "_combofsystrt"))],
                    value3 = get(paste0("f30_", i, "_combofsystrt")),
                    status = ifelse(value3 == 3, 'PASS','FAIL'))
  
  if(nrow(data_tmp) != 0)
    data_rule53_2 <- rbind.fill(data_rule53_2, data_tmp)
}

### Rule #53.3 --------------------------------------------------------------------
# if regimen = cetuximab + other regimen [f36_1_regimen = 15, 18, 21, 80, 31, 35, 38], systemic treatment must be chemo +/- cetuximab [f30_1_combofsystrt = 1]
data_rule53_3 <- data.frame()

var_tmp <- colnames(redcap_data)[grepl("f36_[[:digit:]]_regimen", colnames(redcap_data))]
metadata_tmp <- subset(metadata, field_name %in% var_tmp)

for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & get(metadata_tmp$field_name[i]) %in% c(15,18,21,80,31,35,38) & !(is.na(get(paste0("f30_", i, "_combofsystrt"))))), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Regimen is cetuximab + other regimen but systemic treatment is not Chemotherapy +/- Cetuximab",
                    form1 = "Cancer Under Study Treatment",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    ...
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == paste0("f30_", i, "_combofsystrt"))],
                    value2 = get(paste0("f30_", i, "_combofsystrt")),
                    status = ifelse(value2 == 1, 'PASS','FAIL'))
  
  if(nrow(data_tmp) != 0)
    data_rule53_3 <- rbind.fill(data_rule53_3, data_tmp)
}
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & !(is.na(get(metadata_tmp$field_name[i]))) & !(is.na(get(paste0("f34_", i, "_enddate_syst"))))), c("a01_id"), summarize,
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(paste0("f48_", i, "_reg2stdate_syst_unk")) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Cancer Under Study Treatment",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == paste0("f34_", i, "_enddate_syst"))],
                    value2 = ifelse(!is.na(get(paste0("f34_", i, "_enddate_syst_unk"))) & format(get(paste0("f34_", i, "_enddate_syst")), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(paste0("f34_", i, "_enddate_syst")))), get(paste0("f34_", i, "_enddate_syst"))),
                    status = ifelse(value1 > value2, 'PASS','FAIL'))
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, f107_complete == 1 & !(is.na(get(metadata_tmp$field_name[i]))) & !(is.na(get(paste0("f48_", i, "_reg2stdate_syst"))))), c("a01_id"), summarize,
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(paste0("f49_", i, "_reg2endate_syst_unk")) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Cancer Under Study Treatment",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == paste0("f48_", i, "_reg2stdate_syst"))],
                    value2 = ifelse(!is.na(get(paste0("f48_", i, "_reg2stdate_syst_unk"))) & format(get(paste0("f48_", i, "_reg2stdate_syst")), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(paste0("f48_", i, "_reg2stdate_syst")))), get(paste0("f48_", i, "_reg2stdate_syst"))),
                    status = ifelse(value1 > value2, 'PASS','FAIL'))
...
### Rule #56 --------------------------------------------------------------------
# f54_radiointent is Curative and if  f55_radiosett is Postoperative (3), f63_totdose >= 60
data_rule56 <- ddply(subset(redcap_data, f107_complete == 1 & f54_radiointent == 2 & f55_radiosett == 3 & !is.na(f63_totdose)), c("a01_id"), summarize,
                     category = "ERROR",
                     description = "Postoperative radiotherapy intent is curative but total dose is lower than 60",
                     form1 = "Cancer Under Study Treatment",
                     event1 = "NON REPEATABLE",
                     instance1 = redcap_repeat_instance,
                     var1 = metadata$field_label[which(metadata$field_name == "f63_totdose")],
                     value1 = f63_totdose,
                     status = ifelse(value1 >= 60, 'PASS','FAIL'))

# f54_radiointent is Curative and and if  f55_radiosett is Definitive (5), ), f63_totdose >= 66
data_rule56_2 <- ddply(subset(redcap_data, f107_complete == 1 & f54_radiointent == 2 & f55_radiosett == 5 & !is.na(f63_totdose)), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Definitive radiotherapy intent is curative but total dose is lower than 66",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f63_totdose")],
                       value1 = f63_totdose,
                       status = ifelse(value1 >= 66, 'PASS','FAIL'))
...
data_rule57 <- ddply(subset(redcap_data, f107_complete == 1 & f55_radiosett %in% c(1,2,3,4)), c("a01_id"), summarize,
                     category = "ERROR",
...
### Rule #58 --------------------------------------------------------------------
# if f55_radiosett Preoperative concomitant to systemic treatment;Postoperative concomitant to systemic treatment at least one systemic treatment (f29_1_systrt or f29_2_systrt or f29_3_systrt) should be yes and concomitant

data_rule58 <- ddply(subset(redcap_data, f107_complete == 1 & f55_radiosett %in% c(2,4)), c("a01_id"), plyr::summarize,
                     category = "ERROR",
                     description = "Radiotherapy settings are preoperative or postoperative concomitant to systemic treatment, but concomitant systemic treatment is not performed",
                     form1 = "Cancer Under Study Treatment",
                     event1 = "NON REPEATABLE",
                     instance1 = redcap_repeat_instance,
                     var1 = metadata$field_label[which(metadata$field_name == "f55_radiosett")],
                     value1 = f55_radiosett,
                     status = ifelse(any(mget(grep("f32_[[:digit:]]_systsetting", colnames(redcap_data), value = TRUE), inherits = TRUE) == 2) , 'PASS','FAIL'))

### Rule #59 --------------------------------------------------------------------
# if f55_radiosett Definitive concomitant to systemic treatment at least one concomitant systemic treatment

data_rule59 <- ddply(subset(redcap_data, f107_complete == 1 & f55_radiosett == 6), c("a01_id"), plyr::summarize,
                     category = "ERROR",
                     description = "Radiotherapy settings are definitive concomitant to systemic treatment, but concomitant systemic treatment is not performed",
                     form1 = "Cancer Under Study Treatment",
                     event1 = "NON REPEATABLE",
                     instance1 = redcap_repeat_instance,
                     var1 = metadata$field_label[which(metadata$field_name == "f55_radiosett")],
...
### Rule #60 --------------------------------------------------------------------
# if f55_radiosett Definitive;  Definitive concomitant to systemic treatment the surgery (f01_surgery) should be no

data_rule60 <- ddply(subset(redcap_data, f107_complete == 1 & f55_radiosett %in% c(5,6)), c("a01_id"), summarize,
                     category = "ERROR",
                     description = "Radiotherapy settings are definitive, but surgery is performed",
                     form1 = "Cancer Under Study Treatment",
                     event1 = "NON REPEATABLE",
                     instance1 = redcap_repeat_instance,
                     var1 = metadata$field_label[which(metadata$field_name == "f55_radiosett")],
...
### Rule #60.2 --------------------------------------------------------------------
# If Radiotherapy is done [1,2], f63_totdose must be compiled 

data_rule60_2 <- ddply(subset(redcap_data, f107_complete == 1 & !is.na(f53_radio) & f53_radio %in% c(1,2) & is.na(f63_totdose)), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Radiotherapy is done but total dose is not specified",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f53_radio")],
                       value1 = f53_radio,
                       form2 = "Cancer Under Study Treatment",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "f63_totdose")],
                       value2 = f63_totdose,
                       status = 'FAIL')

### Rule #60.3 --------------------------------------------------------------------
# If Radiotherapy Treatment plan is SEQ or SIB+SEQ [1,4], f63_totdose_2 must be compiled 

data_rule60_3 <- ddply(subset(redcap_data, f107_complete == 1 & !is.na(f61_boost) & f61_boost %in% c(1,4) & is.na(f63_totdose_2)), c("a01_id"), summarize,
                       category = "ERROR",
                       description = "Radiotherapy treatment plan is SEQ or SIB+SEQ but the second total dose is not specified",
                       form1 = "Cancer Under Study Treatment",
                       event1 = "NON REPEATABLE",
                       instance1 = redcap_repeat_instance,
                       var1 = metadata$field_label[which(metadata$field_name == "f61_boost")],
                       value1 = f61_boost,
                       form2 = "Cancer Under Study Treatment",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = metadata$field_label[which(metadata$field_name == "f63_totdose_2")],
                       value2 = f63_totdose_2,
                       status = 'FAIL')
...
data_rule61 <- ddply(subset(redcap_data, f107_complete == 1 & d34_complete == 1 & !(is.na(f80_radiostartdate)) & !(is.na(d01_diagdate))), c("a01_id"), summarize,
                     ...
                     value1 = ifelse(!is.na(f80_radiostartdate_unk) & format(f80_radiostartdate, "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", f80_radiostartdate)),length=2,by="months")[2]-1, f80_radiostartdate),
                     form2 = "Cancer under study",
                     event2 = "NON REPEATABLE",
                     instance2 = redcap_repeat_instance,
                     var2 = metadata$field_label[which(metadata$field_name == "d01_diagdate")],
...
data_rule62 <- ddply(subset(redcap_data, f107_complete == 1 & f55_radiosett %in% c(1,2) & !(is.na(f02_datesurg)) & !(is.na(f81_radioendate))), c("a01_id"), summarize,
                       ...
                       value1 = ifelse(!is.na(f81_radioendate_unk) & format(f81_radioendate, "%d") == "15", as.Date(sub("\\d{2}$", "01", f81_radioendate)), f81_radioendate),
                       form2 = "Cancer Under Study Treatment",
                       event2 = "NON REPEATABLE",
                       instance2 = redcap_repeat_instance,
                       var2 = gsub(" \\(.*?\\)","",gsub("<.*?>","",metadata$field_label[which(metadata$field_name == "f02_datesurg")])),
                       value2 = ifelse(!is.na(f02_datesurg_unk) & format(f02_datesurg, "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", f02_datesurg)),length=2,by="months")[2]-1, f02_datesurg),
                       status = ifelse(value1 < value2, 'PASS','FAIL'))
...
data_rule64 <- ddply(subset(redcap_data, f107_complete == 1 & !(is.na(f80_radiostartdate)) & !(is.na(f81_radioendate))), c("a01_id"), summarize,
                     ...
                     instance1 = redcap_repeat_instance,
                     var1 = metadata$field_label[which(metadata$field_name == "f81_radioendate")],
                     value1 = ifelse(!is.na(f81_radioendate_unk) & format(f81_radioendate, "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", f81_radioendate)),length=2,by="months")[2]-1, f81_radioendate),
                     form2 = "Cancer Under Study Treatment",
                     event2 = "NON REPEATABLE",
                     instance2 = redcap_repeat_instance,
                     var2 = metadata$field_label[which(metadata$field_name == "f80_radiostartdate")],
...
### Rule #65 --------------------------------------------------------------------
# f85_radiosite___5 = 1 only if (e33_cm = 2  or e53_pm = 2) or (e34_cstage or e54_pstage = 8) or (Nasopahrynx [d11_siterare, d11_sitecomrar] and e34_cstage or e54_pstage = 7) or (Oropharynx [d11_siterare, d11_sitecomrar] con d27_p16 = 1  and (e34_cstage or e54_pstage = 5)
...
data_rule65 <- ddply(subset(redcap_data, f107_complete == 1 & d34_complete == 1 & e57_complete == 1 & f85_radiosite___5 == 1), c("a01_id"), summarize,
                     ...
                     status = ifelse(any(c(e33_cm, e53_pm) == 2) | any(c(e34_cstage, e54_pstage) == 8) | 
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & !is.na(get(paste0("g", i, "_03_date")))), c("a01_id"), summarize,
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("g", i, "_09_datesurg_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & !is.na(get(paste0("g", i, "_03_date")))), c("a01_id"), summarize,
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("g", i, "_16_datenecksurg_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Progression/recurrence/persistent Disease",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "g1_03_date")],
...
### Rule #72 -------------------------------------------------------------------
# Number of positive lymph nodes (right side) <= than removed
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & !is.na(get(paste0("g", i, "_20_numlympr")))), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Number of positive lymph nodes (right side) is greater than number of removed",
                    form1 = "Progression/recurrence/persistent Disease",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
...
### Rule #73 -------------------------------------------------------------------
# Number of positive lymph nodes (left side) <= than removed
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & !is.na(get(paste0("g", i, "_22_numlympl")))), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Number of positive lymph nodes (left side) is greater than number of removed",
                    form1 = "Progression/recurrence/persistent Disease",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & !is.na(get(paste0("g", i, "_03_date")))), c("a01_id"), summarize,
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("g", i, "_32_datesurgm_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Progression/recurrence/persistent Disease",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "g1_03_date")],
...
for (i in 1:nrow(metadata_tmp)) {
  ...
  for (j in 1:(nrow(metadata_tmp2) - 1)) {
    data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & get(paste0("g", i, "_38_", j,"_systsetting")) == 4), c("a01_id"), summarize,
                      category = "ERROR",
                      description = "Systemic treatment setting at progression is alone but surgery or radiotherapy have been performed",
                      form1 = "Progression/recurrence/persistent Disease",
                      event1 = "NON REPEATABLE",
                      instance1 = redcap_repeat_instance,
                      var1 = metadata_tmp2$field_label[j],
                      value1 = get(metadata_tmp2$field_name[j]),
                      status = ifelse(!any(mget(c(paste0("g",i,"_08_surgery"), paste0("g",i,"_63_radio")), inherits = TRUE) %in% c(1,2)), 'PASS', 'FAIL'))
...
for (i in 1:nrow(metadata_tmp)) {
  ...
  for (j in 1:nrow(metadata_tmp2)) {
    data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & get(paste0("g", i, "_38_", j,"_systsetting")) == 1 & !(is.na(get(metadata_tmp2$field_name[j]))) & (!is.na(get(paste0("g", i, "_09_datesurg"))) | !is.na(get(paste0("g", i, "_92_radiostartdate"))))), c("a01_id"), summarize,
                      ...
                      instance1 = redcap_repeat_instance,
                      var1 = metadata_tmp2$field_label[j],
                      value1 = ifelse(!is.na(get(paste0("g", i, "_39_", j,"_startdate_syst_unk"))) & format(get(metadata_tmp2$field_name[j]), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(metadata_tmp2$field_name[j]))), get(metadata_tmp2$field_name[j])),
                      form2 = "Progression/recurrence/persistent Disease",
                      event2 = "NON REPEATABLE",
                      instance2 = redcap_repeat_instance,
...
for (i in 1:nrow(metadata_tmp)) {
  ...
  for (j in 1:nrow(metadata_tmp2)) {
    data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & get(paste0("g", i, "_38_", j,"_systsetting")) == 3 & !(is.na(get(metadata_tmp2$field_name[j]))) & (!is.na(get(paste0("g", i, "_09_datesurg"))) | !is.na(get(paste0("g", i, "_93_radioendate"))))), c("a01_id"), summarize,
                      ...
                      instance1 = redcap_repeat_instance,
                      var1 = metadata_tmp2$field_label[j],
                      value1 = ifelse(!is.na(get(paste0("g", i, "_39_", j,"_startdate_syst_unk"))) & format(get(metadata_tmp2$field_name[j]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp2$field_name[j]))),length=2,by="months")[2]-1, get(metadata_tmp2$field_name[j])),
                      form2 = "Progression/recurrence/persistent Disease",
                      event2 = "NON REPEATABLE",
                      instance2 = redcap_repeat_instance,
...
for (i in 1:nrow(metadata_tmp)) {
  ...
  for (j in 1:nrow(metadata_tmp2)) {
    data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & get(paste0("g", i, "_38_", j,"_systsetting")) == 2 & !(is.na(get(metadata_tmp2$field_name[j]))) & get(paste0("g", i, "_67_radiosett")) %in% c(2,4,6) & (!is.na(get(paste0("g", i, "_92_radiostartdate"))) | !is.na(get(paste0("g", i, "_93_radioendate"))))), c("a01_id"), summarize,
                      category = "ERROR",
                      description = "Concomitant systemic treatment at progression/recurrence/persistent disease starts more than 7 days before radiotherapy",
                      form1 = "Progression/recurrence/persistent Disease",
                      event1 = "NON REPEATABLE",
                      instance1 = redcap_repeat_instance,
                      var1 = metadata_tmp2$field_label[j],
                      value1 = get(metadata_tmp2$field_name[j]),
                      value1_up = ifelse(!is.na(get(paste0("g", i, "_39_", j,"_startdate_syst_unk"))) & format(get(metadata_tmp2$field_name[j]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp2$field_name[j]))),length=2,by="months")[2]-1, get(metadata_tmp2$field_name[j])),
                      form2 = "Progression/recurrence/persistent Disease",
                      event2 = "NON REPEATABLE",
                      instance2 = redcap_repeat_instance,
                      ...
                      status = ifelse(value1_up >= ifelse(get(paste0("g", i,"_36_", j,"_combofsystrt")) == 1, value2 - 7, value2), 'PASS','FAIL'))
...
for (i in 1:nrow(metadata_tmp)) {
  ...
  for (j in 1:nrow(metadata_tmp2)) {
    data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !(is.na(get(metadata_tmp2$field_name[j]))) & !is.na(get(paste0("g", i, "_03_date")))), c("a01_id"), summarize,
                      category = "ERROR",
...
for (i in 1:nrow(metadata_tmp)) {
  ...
  for (j in 1:nrow(metadata_tmp2)) {
    data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !(is.na(get(metadata_tmp2$field_name[j]))) & !is.na(get(paste0("g", i, "_39_", j,"_startdate_syst")))), c("a01_id"), summarize,
                      category = "ERROR",
                      ...
                      instance1 = redcap_repeat_instance,
                      var1 = metadata_tmp2$field_label[j],
                      value1 = ifelse(!is.na(get(paste0("g", i, "_40_", j,"_enddate_syst_unk"))) & format(get(metadata_tmp2$field_name[j]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp2$field_name[j]))),length=2,by="months")[2]-1, get(metadata_tmp2$field_name[j])),
                      form2 = "Progression/recurrence/persistent Disease",
                      event2 = "NON REPEATABLE",
                      instance2 = redcap_repeat_instance,
                      var2 = metadata$field_label[which(metadata$field_name == "g1_39_1_startdate_syst")],
                      value2 = ifelse(!is.na(get(paste0("g", i, "_39_", j,"_startdate_syst_unk"))) & format(get(paste0("g", i, "_39_", j,"_startdate_syst")), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(paste0("g", i, "_39_", j,"_startdate_syst")))), get(paste0("g", i, "_39_", j,"_startdate_syst"))),
                      status = ifelse(value1 > value2,'PASS','FAIL'))
...
for (i in 1:nrow(metadata_tmp)) {
  ...
  for (j in 1:nrow(metadata_tmp2)) {
    data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !(is.na(get(metadata_tmp2$field_name[j]))) & !is.na(get(paste0("g", i, "_40_", j,"_enddate_syst")))), c("a01_id"), summarize,
                      ...
                      instance1 = redcap_repeat_instance,
                      var1 = metadata_tmp2$field_label[j],
                      value1 = ifelse(!is.na(get(paste0("g", i, "_58_", j,"_reg2startdate_syst_unk"))) & format(get(metadata_tmp2$field_name[j]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp2$field_name[j]))),length=2,by="months")[2]-1, get(metadata_tmp2$field_name[j])),
                      form2 = "Progression/recurrence/persistent Disease",
                      event2 = "NON REPEATABLE",
                      instance2 = redcap_repeat_instance,
                      var2 = metadata$field_label[which(metadata$field_name == "g1_40_1_enddate_syst")],
                      value2 = ifelse(!is.na(get(paste0("g", i, "_40_", j,"_enddate_syst_unk"))) & format(get(paste0("g", i, "_40_", j,"_enddate_syst")), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(paste0("g", i, "_40_", j,"_enddate_syst")))), get(paste0("g", i, "_40_", j,"_enddate_syst"))),
                      status = ifelse(value1 > value2,'PASS','FAIL'))
...
for (i in 1:nrow(metadata_tmp)) {
  ...
  for (j in 1:nrow(metadata_tmp2)) {
    data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !(is.na(get(metadata_tmp2$field_name[j]))) & !is.na(get(paste0("g", i, "_58_", j,"_reg2startdate_syst")))), c("a01_id"), summarize,
                      ...
                      instance1 = redcap_repeat_instance,
                      var1 = metadata_tmp2$field_label[j],
                      value1 = ifelse(!is.na(get(paste0("g", i, "_59_", j,"_reg2enddate_syst_unk"))) & format(get(metadata_tmp2$field_name[j]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp2$field_name[j]))),length=2,by="months")[2]-1, get(metadata_tmp2$field_name[j])),
                      form2 = "Progression/recurrence/persistent Disease",
                      event2 = "NON REPEATABLE",
                      instance2 = redcap_repeat_instance,
                      var2 = metadata$field_label[which(metadata$field_name == "g1_58_1_reg2startdate_syst")],
                      value2 = ifelse(!is.na(get(paste0("g", i, "_58_", j,"_reg2startdate_syst_unk"))) & format(get(paste0("g", i, "_58_", j,"_reg2startdate_syst")), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(paste0("g", i, "_58_", j,"_reg2startdate_syst")))), get(paste0("g", i, "_58_", j,"_reg2startdate_syst"))),
                      status = ifelse(value1 > value2,'PASS','FAIL'))
...
### Rule #91 -------------------------------------------------------------------
# if radiotherapy definitve surgery should be no
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & get(metadata_tmp$field_name[i]) %in% c(5,6)), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Radiotherapy is definitive but surgery is performed",
                    form1 = "Progression/recurrence/persistent Disease",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = get(metadata_tmp$field_name[i]),
                    status = ifelse(get(paste0("g", i, "_08_surgery")) %in% c(1,2), 'FAIL', 'PASS'))
...
### Rule #92 -------------------------------------------------------------------
# if radiotherapy concomitant to sys trt, systemic treatment should be yes and concomitant
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & get(metadata_tmp$field_name[i]) %in% c(2,4,6)), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Radiotherapy is concomitant to systemic treatment but concomitant treatment is not defined",
                    form1 = "Progression/recurrence/persistent Disease",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & get(paste0("g", i, "_67_radiosett")) %in% c(1,2) & !is.na(get(paste0("g", i, "_09_datesurg")))), c("a01_id"), summarize,
                    category = "ERROR",
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("g", i, "_93_radioendate_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))), get(metadata_tmp$field_name[i])),
                    form2 = "Progression/recurrence/persistent Disease",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "g1_09_datesurg")],
                    value2 = ifelse(!is.na(get(paste0("g", i, "_09_datesurg_unk"))) & format(get(paste0("g", i, "_09_datesurg")), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(paste0("g", i, "_09_datesurg")))),length=2,by="months")[2]-1, get(paste0("g", i, "_09_datesurg"))),
                    status = ifelse(value1 < value2, 'PASS', 'FAIL'))
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & get(paste0("g", i, "_67_radiosett")) %in% c(3,4) & !is.na(get(paste0("g", i, "_09_datesurg")))), c("a01_id"), summarize,
                    category = "ERROR",
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("g", i, "_92_radiostartdate_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Progression/recurrence/persistent Disease",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "g1_09_datesurg")],
                    value2 = ifelse(!is.na(get(paste0("g", i, "_09_datesurg_unk"))) & format(get(paste0("g", i, "_09_datesurg")), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(paste0("g", i, "_09_datesurg")))), get(paste0("g", i, "_09_datesurg"))),
                    status = ifelse(value1 > value2, 'PASS', 'FAIL'))
...
### Rule #94.2 -------------------------------------------------------------------
#  If Radiotherapy (g*_63_radio) is done [1,2], g*_75_totd must be compiled
data_rule94_2 <- data.frame()

var_tmp <- colnames(redcap_data)[grepl("g[[:digit:]]+_63_radio", colnames(redcap_data)) & !grepl("unk", colnames(redcap_data))]
metadata_tmp <- subset(metadata, field_name %in% var_tmp)

for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & get(metadata_tmp$field_name[i]) %in% c(1,2) & is.na(get(paste0("g", i, "_75_totd")))), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Radiotherapy at progression/recurrence is done but total dose is not specified",
                    form1 = "Progression/recurrence/persistent Disease",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    ...
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "g1_75_totd")],
                    value2 = get(paste0("g", i, "_75_totd")),
                    status = 'FAIL')
  
  if(nrow(data_tmp) != 0)
    data_rule94_2 <- rbind.fill(data_rule94_2, data_tmp)
}

### Rule #94.3 -------------------------------------------------------------------
#  # If Radiotherapy Treatment plan (g*_73_boost) is SEQ or SIB+SEQ [1,4], g*_75_totd_2 must be compiled
data_rule94_3 <- data.frame()

var_tmp <- colnames(redcap_data)[grepl("g[[:digit:]]+_73_boost", colnames(redcap_data)) & !grepl("unk", colnames(redcap_data))]
metadata_tmp <- subset(metadata, field_name %in% var_tmp)

for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & get(metadata_tmp$field_name[i]) %in% c(1,4) & is.na(get(paste0("g", i, "_75_totd_2")))), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Radiotherapy treatment plan at progression/recurrence is SEQ or SIB+SEQ but the second total dose is not specified",
                    form1 = "Progression/recurrence/persistent Disease",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    ...
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "g1_75_totd_2")],
                    value2 = get(paste0("g", i, "_75_totd_2")),
                    status = 'FAIL')
  
  if(nrow(data_tmp) != 0)
    data_rule94_3 <- rbind.fill(data_rule94_3, data_tmp)
}
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & !is.na(get(paste0("g", i, "_92_radiostartdate")))), c("a01_id"), summarize,
                    category = "ERROR",
                    description = "Radiotherapy end date is prior to start date",
                    form1 = "Progression/recurrence/persistent Disease",
                    event1 = "NON REPEATABLE",
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("g", i, "_93_radioendate_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Progression/recurrence/persistent Disease",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "g1_92_radiostartdate")],
                    value2 = ifelse(!is.na(get(paste0("g", i, "_92_radiostartdate_unk"))) & format(get(paste0("g", i, "_92_radiostartdate")), "%d") == "15", as.Date(sub("\\d{2}$", "01", get(paste0("g", i, "_92_radiostartdate")))), get(paste0("g", i, "_92_radiostartdate"))),
                    status = ifelse(value1 >= value2, 'PASS', 'FAIL'))
...
for (i in 1:nrow(metadata_tmp)) {
  data_tmp <- ddply(subset(redcap_data, get(paste0("g", i, "_119_complete")) == 1 & !is.na(get(metadata_tmp$field_name[i])) & !is.na(get(paste0("g", i, "_03_date")))), c("a01_id"), summarize,
                    ...
                    instance1 = redcap_repeat_instance,
                    var1 = metadata_tmp$field_label[i],
                    value1 = ifelse(!is.na(get(paste0("g", i, "_112_othertrt_date_unk"))) & format(get(metadata_tmp$field_name[i]), "%d") == "15", seq(as.Date(sub("\\d{2}$", "01", get(metadata_tmp$field_name[i]))),length=2,by="months")[2]-1, get(metadata_tmp$field_name[i])),
                    form2 = "Progression/recurrence/persistent Disease",
                    event2 = "NON REPEATABLE",
                    instance2 = redcap_repeat_instance,
                    var2 = metadata$field_label[which(metadata$field_name == "g1_03_date")],
...
data_rule101 <- ddply(subset(redcap_data, h06_complete == 1 & !(is.na(h02_datelasfup))), c("a01_id"), summarize,
                      category = "ERROR",
                      description = "Date of last follow-up is prior to last date of progression/recurrence/persistent disease",
                      form1 = "Status of patient at fup",
                      event1 = "NON REPEATABLE",
                      instance1 = redcap_repeat_instance,
                      var1 = metadata$field_label[which(metadata$field_name == "h02_datelasfup")],
...
# core variables - unknown 
## Define core variables
core_var <- c("a05_date", "a06_phase", "b04_sex", "b06_resid", "b07_smoke", "b11_smokepy_auto", "b13_alc", "b19_comorb",
              ...
              "d18_subsite_lar", "d19_subsite_oc", "d20_subsite_lip", "d21_ebv", "e01_stagehosp", "e02_im_primary", "e12_im_neck", "e21_im_m",
...
data_core_unk <- data.frame()

for (i in 1:nrow(metadata_core)) {
  var_tmp = metadata_core$field_name[i]
  form_complete = ifelse(substring(var_tmp,1,1) == "g",
                         grep(paste0(strsplit(var_tmp, split = "_")[[1]][1],"_[[:digit:]]+_complete"), metadata$field_name, value = TRUE),
                         grep(paste0(substring(var_tmp,1,1),"[[:digit:]]+_complete"), metadata$field_name, value = TRUE))
  expr_str = ifelse(!is.na(metadata_core$branching_logic[i]), renderLogic(metadata_core$branching_logic[i]), "TRUE")
  
  if(metadata_core$field_type[i] != "checkbox"){
    data_tmp <- ddply(subset(redcap_data, get(form_complete) == 1 & eval(parse(text = expr_str))), c("a01_id"), summarize,
                      category = ifelse(is.na(get(var_tmp)),"MISSING","UNKNOWN"),
                      description = paste(category,var_tmp, sep = " - "),
                      form1 = NA,
                      event1 = "NON REPEATABLE",
                      instance1 = redcap_repeat_instance,
                      var1 = var_tmp,
                      value1 = as.character(get(var_tmp)),
                      status = ifelse(value1 %in% c(999, 9999) | is.na(value1), 'FAIL', 'PASS'))
  } else {
    data_tmp <- ddply(subset(redcap_data, get(form_complete) == 1 & eval(parse(text = expr_str))), c("a01_id"), summarize,
                      category = ifelse(all(unlist(mget(grep(paste0(var_tmp, "___"), colnames(redcap_data), value = TRUE), inherits = TRUE)) == 0),"MISSING","UNKNOWN"),
                      description = paste(category,var_tmp, sep = " - "),
                      form1 = NA,
                      event1 = "NON REPEATABLE",
                      instance1 = redcap_repeat_instance,
                      var1 = var_tmp,
                      value1 = ifelse(length(grep(paste0(var_tmp, "___999"), colnames(redcap_data))), get(paste0(var_tmp, "___999")), 0),
                      status = ifelse(value1 == 1 | all(unlist(mget(grep(paste0(var_tmp, "___"), colnames(redcap_data), value = TRUE), inherits = TRUE)) == 0), 'FAIL', 'PASS'))
  }
  
  if(nrow(data_tmp) != 0)
    data_core_unk <- rbind.fill(data_core_unk, data_tmp)
} 

data_core_unk <- data_core_unk %>%
  subset(status == 'FAIL') %>%
  select(-c(grep("value", colnames(.)), status))
...
if(properties$id_centro == id_aiocc){
  ...
  for (i in dags) {
    ...
    addWorksheet(wb, paste0(toupper(i),"-Core"))
    
    check_dag_i <- check_dag %>%
      filter(redcap_data_access_group == i)
    
    core_unk_dag_i <- core_unk_dag %>%
      filter(redcap_data_access_group == i)
    ...
    if(!(empty(check_dag_i))){
      ...
      data.table::setnames(err_regole, old = c("category","description", "freq"), new = c("Error category", "Error description", "Total"))
      ...
      columnNames <- colnames(legend) %>% 
        ...
        gsub("var", "Variable", .) %>%
        gsub("event", "Event", .) %>% 
        gsub("form", "Instrument", .) %>% 
        gsub("category", "Error category", .) %>%
        gsub("description", "Error description", .)
      ...
      addFilter(wb, sheet = paste0(toupper(i),"-Checks"), row = 1, cols = 1:ncol(err_regole))
      ...
      writeData(wb,sheet = paste0(toupper(i),"-Legend"), legend, borders = "all", borderColour = "#999999",
                headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))                             
      addFilter(wb, sheet = paste0(toupper(i),"-Legend"), row = 1, cols = 1:ncol(legend))
    ...
    if(!(empty(core_unk_dag_i))){
      # if not empty build new dataframe
      err_core <- summary.rules(core_unk_dag_i, method = "patients") %>%
        mutate(description = gsub('UNKNOWN - |MISSING - ', '', description)) %>%
        arrange(match(description, metadata$field_name)) %>%
        left_join(., metadata_core[,c("field_name", "form_name", "field_label")], by = c("description" = "field_name")) %>%
        mutate(instrument_label = case_when(form_name == "preliminary" ~ "1.Preliminary",
                                            form_name == "demographic_life_style" ~ "2.Demographic & life style",
                                            form_name == "previous_cancer_gensyndrome" ~ "3.Previous cancer gen.syndrome",
                                            form_name == "cancer_under_study" ~ "4.Cancer under study",
                                            form_name == "staging_procedures_and_stage" ~ "5.Staging procedures and stage",
                                            form_name == "cancer_under_study_treatment" ~ "6.Cancer Under Study Treatment",
                                            form_name == "progressionrecurrencepersistent_disease" ~ "7.1.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_a9bc" ~ "7.2.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_1aa9" ~ "7.3.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_d93f" ~ "7.4.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_f74f" ~ "7.5.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_3672" ~ "7.6.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_1bf2" ~ "7.7.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_aba2" ~ "7.8.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_5f7c" ~ "7.9.Progression/recurrence/persistent Disease",
                                            form_name == "progressionrecurrencepersistent_disease_859b" ~ "7.10.Progression/recurrence/persistent Disease",
                                            form_name == "status_of_patient_at_fup" ~ "8.Status of patient at fup")) %>%
        mutate(description = ifelse(description != 'Total', paste(description, gsub("<.*?>","",field_label), sep = ' - '), description)) %>%
        select(category, description, instrument_label, everything()) %>%
        select(-c(form_name, field_label))
      
      # set new columns names
      data.table::setnames(err_core, old = c("category","description","instrument_label","freq"), new = c("Value", "Variable", "Instrument","Total"))

      # write data on worksheet
      writeData(wb,sheet = paste0(toupper(i),"-Core"), err_core, borders = "all", borderColour = "#999999",
                headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))                             
      addFilter(wb, sheet = paste0(toupper(i),"-Core"), row = 1, cols = 1:ncol(err_core))
      setColWidths(wb,sheet = paste0(toupper(i),"-Core"),cols = 1:ncol(err_core),widths = "auto")
    } else {
      # if empty write "No core variables flagged as unknown" on the excel file
      writeData(wb,sheet = paste0(toupper(i),"-Core"), "No core variables flagged as unknown") 
    }
  ...
  if(!(empty(check_failed))){
    ...
    writeData(wb,sheet = "Checks", err_regole, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold")) 
    addFilter(wb, sheet = "Checks", row = 1, cols = 1:ncol(err_regole))
    ...
    writeData(wb,sheet = "Legend", legend, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold")) 
    addFilter(wb, sheet = "Legend", row = 1, cols = 1:ncol(legend))
  ...
  # Core variables unknown
  if(!(empty(data_core_unk))){
    # if not empty build new dataframe
    err_core <- summary.rules(data_core_unk, method = "patients") %>% 
      mutate(description = gsub('UNKNOWN - |MISSING - ', '', description)) %>%
      arrange(match(description, metadata$field_name)) %>%
      left_join(., metadata_core[,c("field_name", "form_name", "field_label")], by = c("description" = "field_name")) %>%
      mutate(instrument_label = case_when(form_name == "preliminary" ~ "1.Preliminary",
    ...
    # write data on worksheet
    writeData(wb,sheet = "Core", err_core, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))  
    addFilter(wb, sheet = "Core", row = 1, cols = 1:ncol(err_core))
    setColWidths(wb,sheet = "Core",cols = 1:ncol(err_core),widths = "auto")
  } else {
    # if empty write "No core variables flagged as unknown" on the excel file
    writeData(wb,sheet = "Core", "No core variables flagged as unknown") 
  }
}
...
############################################## QC REPORT SUMMARY ##############################################
# Histology
histology <- metadata[which(metadata$field_name == "d05_histo"),]$select_choices_or_calculations %>%
  strsplit(., "\\|") %>%
  unlist(.)
...
for (i in histology) {
  ...
  code <- c(code, as.integer(parts[1]))
  desc <- c(desc, parts[2])
}

histology <- df <- data.frame(Code = code, Histology = desc, stringsAsFactors = FALSE)

# Site 
sitecomrare <- metadata[which(metadata$field_name == "d11_sitecomrar"),]$select_choices_or_calculations %>%
  strsplit(., "\\|") %>%
  unlist(.)
...
for (i in sitecomrare) {
  parts <- unlist(strsplit(i, split = ", "))
  code <- c(code, as.integer(parts[1]))
  desc <- c(desc, parts[2])
}

sitecomrare <- df <- data.frame(Code = code, SiteLab = desc, stringsAsFactors = FALSE)
...
if(properties$id_centro == id_aiocc) {
  ...
  for (m in dags) {
    ...
    metadata_tmp <- subset(metadata,!(field_type %in% c("descriptive","calc", "checkbox"))) %>%
      subset(!(field_annotation %in% c("@HIDDEN"))) %>%
    ...
    for (i in 1:nrow(metadata_tmp)){
      ...
      form_complete = ifelse(substring(var_tmp,1,1) == "g",
                             grep(paste0(strsplit(var_patt, split = "_")[[1]][1],"_[[:digit:]]+_complete"), metadata$field_name, value = TRUE),
    ...
    check_summary <- check_summary %>%
      arrange(match(var, metadata$field_name)) %>%
      mutate(isCore = ifelse(var %in% core_var, "X", "")) %>% #add a column with "X" if the variable is in the list of core variables
      ...
      select(-c("var")) %>%
      select(c(var_lab, isCore), everything()) %>%
      mutate(var_lab = gsub("<.*?>", "", var_lab)) %>%
      filter(var_lab != "Instrument status:")
    
    colnames(check_summary) <- c("Variable", "Core","Instrument", "Denominator","% missing", "% missing + unknown")
    ...
    colnames(case_complete) <- c("Instrument status", "Description","N pts", "N pts with at least one error", "% pts with at least one error over total n pts")
    
    # Staging & Treatments ---------
    data_trt_complete <- redcap_data_m %>% subset(redcap_event_name == "non_repeatable_arm_1") %>%
      ...
      mutate(treatment = case_when(
        ...
        f30_1_combofsystrt == 1 & !(f01_surgery %in% c(1,2)) & !(f53_radio %in% c(1,2)) & !(f29_2_systrt %in% c(1,2)) & !(f29_3_systrt %in% c(1,2)) & f33_1_startdate_syst <= (d01_diagdate + 90) ~ "Only systemic treatment",
        f30_1_combofsystrt == 2 & !(f01_surgery %in% c(1,2)) & !(f53_radio %in% c(1,2)) & !(f29_2_systrt %in% c(1,2)) & !(f29_3_systrt %in% c(1,2)) & f33_1_startdate_syst <= (d01_diagdate + 90) ~ "Only systemic treatment",
        f30_1_combofsystrt == 3 & !(f01_surgery %in% c(1,2)) & !(f53_radio %in% c(1,2)) & !(f29_2_systrt %in% c(1,2)) & !(f29_3_systrt %in% c(1,2)) & f33_1_startdate_syst <= (d01_diagdate + 90) ~ "Only systemic treatment",
        # SOLO CONCOMITANTI
        ...
        f01_surgery %in% c(1,2) & !(f53_radio %in% c(1,2)) & f30_1_combofsystrt == 1 & f32_1_systsetting == 3 & !(f29_2_systrt %in% c(1,2)) & !(f29_3_systrt %in% c(1,2)) & min(f02_datesurg, f12_datenecksurg, na.rm = TRUE) <= (d01_diagdate + 90) & f33_1_startdate_syst <= (max(f02_datesurg, f12_datenecksurg, na.rm = TRUE) + 90) ~ "Surgery + adjuvant systemic treatment",
        ...
        # RADIO + altro
        !(f01_surgery %in% c(1,2)) & f53_radio %in% c(1,2) & f30_1_combofsystrt == 1 & f32_1_systsetting == 3 & !(f29_2_systrt %in% c(1,2)) & !(f29_3_systrt %in% c(1,2)) & f80_radiostartdate <= (d01_diagdate + 90) & f33_1_startdate_syst <= (f81_radioendate + 45) ~ "Radio + adjuvant systemic treatment",
    ...
    lev_cstage <- factor(unique(data_trt$e34_cstage),
                        levels = c("0", "I", "II", "III", "IV", "IVA", "IVB", "IVC", "Unknown", "Stage not defined"))
    lev_trt <- factor(unique(data_trt$treatment),
                      levels = c("Only surgery", "Only radiotherapy", "Only systemic treatment", "Concomitant chemo/radio", "Surgery + postoperative radiotherapy", "Surgery + adjuvant systemic treatment", "Surgery + Postoperative radio concomitant to chemotherapy", "Radio + adjuvant systemic treatment", "Concomitant chemo/radio + adjuvant chemo",
                                 "Neo-adjuvant chemo + radio", "Neo-adjuvant chemo + surgery", "Neo-adjuvant chemo + concomitant radio/chemo", "Neo-adjuvant chemo + concomitant radio/chemo + chemo adjuvant", "Neo-adjuvant chemo + radio + chemo adjuvant", "Clinical trial only", "No treatment", "Other"))
    ...
    cstage_per_treat_tot <- setNames(vector("list", length(levels(lev_site))), levels(lev_site))
    
    for (k in names(cstage_per_treat_tot)) {
      data_trt_tmp <- data_trt %>%
        filter(site == k)
      
      stage_per_treat <- setNames(data.frame(matrix(ncol = length(levels(lev_trt)), nrow = length(levels(lev_cstage))), row.names = levels(lev_cstage)), levels(lev_trt))
      ...
      cstage_per_treat_tot[[k]] <- stage_per_treat
      
      gc()
    }
    
    # Pathological stage
    data_trt_surg <- data_trt_complete %>%
      filter(f01_surgery %in% c(1,2)) %>% # filtro sui pazienti con chirurgia
      select(c("a01_id", "d11_siterare", "d11_sitecomrar","e54_pstage", "treatment")) %>%
      mutate(e54_pstage = case_when(e54_pstage == 999 ~ "Unknown",
                                    e54_pstage == 1 ~ "0",
                                    e54_pstage == 2 ~ "I",
                                    e54_pstage == 3 ~ "II",
                                    e54_pstage == 4 ~ "III",
      ...
      mutate(site = case_when(d11_siterare == 1 | d11_sitecomrar == 1 ~ "Nasal cavity and paranasal sinuses",
                              ...
                              d11_siterare == 6 | d11_sitecomrar == 6 ~ "Middle ear",
                              TRUE ~ "Other")) %>%
      plyr::count(c("e54_pstage", "treatment", "site")) %>%
      mutate(e54_pstage = ifelse(is.na(e54_pstage), "Stage not defined", e54_pstage))
    
    lev_pstage <- factor(unique(data_trt_surg$e54_pstage),
                         levels = c("0", "I", "II", "III", "IV", "IVA", "IVB", "IVC", "Unknown", "Stage not defined"))
    lev_trt <- factor(unique(data_trt_surg$treatment),
                      levels = c("Only surgery", "Only radiotherapy", "Only systemic treatment", "Concomitant chemo/radio", "Surgery + postoperative radiotherapy", "Surgery + adjuvant systemic treatment", "Surgery + Postoperative radio concomitant to chemotherapy", "Radio + adjuvant systemic treatment", "Concomitant chemo/radio + adjuvant chemo",
                                 "Neo-adjuvant chemo + radio", "Neo-adjuvant chemo + surgery", "Neo-adjuvant chemo + concomitant radio/chemo", "Neo-adjuvant chemo + concomitant radio/chemo + chemo adjuvant", "Neo-adjuvant chemo + radio + chemo adjuvant", "Clinical trial only", "No treatment", "Other"))
    lev_site <- factor(unique(data_trt_surg$site),
                       levels = c("Nasal cavity and paranasal sinuses", "Nasopharynx", "Parotid gland; Submandibular gland; Sublingual gland", "Middle ear", "Other"))
    
    pstage_per_treat_tot <- setNames(vector("list", length(levels(lev_site))), levels(lev_site))
    
    for (k in names(pstage_per_treat_tot)) {
      data_trt_tmp <- data_trt_surg %>%
        filter(site == k)
      
      stage_per_treat <- setNames(data.frame(matrix(ncol = length(levels(lev_trt)), nrow = length(levels(lev_pstage))), row.names = levels(lev_pstage)), levels(lev_trt))
      
      for (i in 1:nrow(stage_per_treat)) {
        stage <- rownames(stage_per_treat)[i]
        for (j in 1:ncol(stage_per_treat)) {
          trt <- colnames(stage_per_treat)[j]
          stage_per_treat[i,j] <- ifelse(length(data_trt_tmp[which(data_trt_tmp$e54_pstage == stage & data_trt_tmp$treatment == trt), 'freq']),
                                         data_trt_tmp[which(data_trt_tmp$e54_pstage == stage & data_trt_tmp$treatment == trt), 'freq'], NA)
        }
      }
      ...
      pstage_per_treat_tot[[k]] <- stage_per_treat
    ...
    data_year_diag <- redcap_data_m %>% subset(redcap_event_name == "non_repeatable_arm_1") %>%
      select(c("a01_id", "d01_diagdate", "d05_histo", "d11_siterare", "d11_sitecomrar", "d34_complete")) %>%
      mutate(site = ifelse(!is.na(d11_siterare), d11_siterare, d11_sitecomrar)) %>%
      mutate(year_diag = format(d01_diagdate, format = '%Y')) %>%
      plyr::count(c("year_diag", "site", "d05_histo", "d34_complete")) %>%
      group_by(year_diag, site, d05_histo) %>%
      summarise(tot = sum(freq), tot_complete = sum(freq[d34_complete == 1]))
    
    data_year_diag <- data_year_diag %>%
      ungroup() %>%
      left_join(sitecomrare, by = c("site" = "Code")) %>%
      left_join(histology, by = c("d05_histo" = "Code")) %>%
      select(year_diag, SiteLab, Histology, tot, tot_complete)
       
    colnames(data_year_diag) <- c("Year of diagnosis", "Site", "Histology group", "Total N of patients", "N of patients with instrum 4 complete")
    ...
    # remove patients with at least one missing or unknown core variable
    core_pts <- redcap_data_m %>% 
    ...
    # Follow-up --------
    fup_data <- redcap_data_m %>%
      filter(!is.na(h02_datelasfup) & !is.na(d01_diagdate)) %>%
      select(d01_diagdate, h01_status, h02_datelasfup) %>% 
      mutate(d01_diagdate = factor(format(d01_diagdate, "%Y")),
             h02_datelasfup = factor(format(h02_datelasfup, "%Y")),
             h01_status = case_when(h01_status == 1 ~ "Alive, No Evidence of Disease (NED)", 
                                    h01_status == 5 ~ "Alive With Disease (AWD)",
                                    h01_status %in% c(2,3,4) ~ "Dead",
                                    TRUE ~ "Not available")) %>%
      count(d01_diagdate, h01_status, h02_datelasfup)
    
    seq_year <- c(2018:format(Sys.Date(), "%Y"))
    
    empty_df <- data.frame(matrix(ncol = length(seq_year), nrow = length(seq_year), dimnames = list (seq_year, seq_year)))
    colnames(empty_df) <- seq_year
    
    final_fup_df <- data.frame()
    
    for (j in c("Alive, No Evidence of Disease (NED)", "Alive With Disease (AWD)", "Dead", "Not available")) {
      fup_df_tmp <- empty_df
      fup_data_j <- fup_data %>% filter(h01_status == j)
      if(nrow(fup_data_j)) {
        for (k in 1:nrow(fup_data_j)) {
          fup_df_tmp[grep(fup_data_j$h02_datelasfup[k], row.names(empty_df), value = TRUE),grep(fup_data_j$d01_diagdate[k], colnames(empty_df), value = TRUE)] <- fup_data_j$n[k]
        }
      }
      fup_df_tmp <- cbind(data.frame("status_fup" = rep(j, length(seq_year))),
                          data.frame("year_fup" = row.names(fup_df_tmp)),
                          fup_df_tmp)
      
      final_fup_df <- rbind.fill(final_fup_df, fup_df_tmp)
    }
    
    final_fup_df <- rbind(colnames(final_fup_df), final_fup_df)
    
    colnames(final_fup_df) <- c("Status at follow up", "Year follow up", rep("Year diagnosis", length(seq_year)))
    
    # Lost at Follow-up --------
    fup_data <- redcap_data_m %>%
      filter(h06_complete == 1 & d34_complete == 1 & h01_status %in% c(1,5) & !is.na(h02b_dropout) & !is.na(d01_diagdate)) %>%
      select(d01_diagdate, h01_status, h02_datelasfup, h02b_dropout) %>% 
      mutate(d01_diagdate = factor(format(d01_diagdate, "%Y")),
             h02_datelasfup = factor(format(h02_datelasfup, "%Y")),
             h01_status = ifelse(h01_status == 1, "Alive, No Evidence of Disease (NED)", "Alive With Disease (AWD)"),
             h02b_dropout = ifelse(h02b_dropout == 1, "Yes", "No")) %>%
      count(d01_diagdate, h01_status, h02_datelasfup, h02b_dropout)
    ...
    lost_fup_df <- data.frame()
    
    for (i in c("Yes", "No")) {
      for (j in c("Alive, No Evidence of Disease (NED)", "Alive With Disease (AWD)")) {
        fup_df_tmp <- empty_df
        fup_data_ij <- fup_data %>% filter(h02b_dropout == i, h01_status == j)
        if(nrow(fup_data_ij)) {
          for (k in 1:nrow(fup_data_ij)) {
            fup_df_tmp[grep(fup_data_ij$h02_datelasfup[k], row.names(empty_df), value = TRUE),grep(fup_data_ij$d01_diagdate[k], colnames(empty_df), value = TRUE)] <- fup_data_ij$n[k]
          }
        }
        fup_df_tmp <- cbind(data.frame("lost_fup" = rep(i, length(seq_year))), 
                            data.frame("status_fup" = rep(j, length(seq_year))),
                            data.frame("year_fup" = row.names(fup_df_tmp)),
                            fup_df_tmp)
        
        lost_fup_df <- rbind.fill(lost_fup_df, fup_df_tmp)
      }
    }
    
    lost_fup_df <- rbind(colnames(lost_fup_df), lost_fup_df)
    
    colnames(lost_fup_df) <- c("Lost at follow up", "Status at follow up", "Year follow up", rep("Year diagnosis", length(seq_year)))
    
    # Max number of repetitions --------
    maxRep <- setNames(data.frame(matrix(ncol = 3, nrow = 0)), c("adverse_events", "imaging_available", "tumor_specimen_available"))
    for (i in colnames(maxRep)) {
      maxRep[1,i] <- ifelse(nrow(filter(redcap_data_m, redcap_repeat_instrument == i) > 0), max(filter(redcap_data_m, redcap_repeat_instrument == i)$redcap_repeat_instance), 0)
    }
    
    # add worksheet
    addWorksheet(wb, paste0(toupper(m),"-Miss&Unk"))
    addWorksheet(wb, paste0(toupper(m),"-Complete"))
    addWorksheet(wb, paste0(toupper(m),"-CStage&Trt"))
    addWorksheet(wb, paste0(toupper(m),"-PStage&Trt"))
    addWorksheet(wb, paste0(toupper(m),"-Diag&Site"))
    addWorksheet(wb, paste0(toupper(m),"-Core"))
    addWorksheet(wb, paste0(toupper(m),"-FUP"))
    addWorksheet(wb, paste0(toupper(m),"-LostFUP"))
    addWorksheet(wb, paste0(toupper(m),"-MaxRep"))
    ...
    # Complete patients
    mergeCells(wb, paste0(toupper(m),"-Complete"), cols = 1, rows = 2:12)
    mergeCells(wb, paste0(toupper(m),"-Complete"), cols = 1, rows = 13:14)
    writeData(wb,sheet = paste0(toupper(m),"-Complete"), case_complete, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
    setColWidths(wb,sheet = paste0(toupper(m),"-Complete"),cols = 1:ncol(case_complete), widths = "auto")
    
    # Clinical Staging & Treatments
    curr_row <- 1
    for(i in seq_along(cstage_per_treat_tot)[-length(seq_along(cstage_per_treat_tot))]) {
      writeData(wb, paste0(toupper(m),"-CStage&Trt"), names(cstage_per_treat_tot)[i], startCol = 1, startRow = curr_row)
      addStyle(wb, sheet = paste0(toupper(m),"-CStage&Trt"), createStyle(textDecoration = "bold", border = "TopBottomLeftRight", borderColour = "#999999", fontSize = 12, fontColour = "#cc0000"), rows = curr_row, cols = 1)
      writeData(wb, paste0(toupper(m),"-CStage&Trt"), cstage_per_treat_tot[[i]], startCol = 1, startRow = curr_row + 1, rowNames = TRUE, borders = "all", borderColour = "#999999",
                headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
      curr_row <- curr_row + nrow(cstage_per_treat_tot[[i]]) + 4
    }
    setColWidths(wb,sheet = paste0(toupper(m),"-CStage&Trt"),cols = 1:ncol(cstage_per_treat_tot[[1]]), widths = "auto")
    
    # Pathological Staging & Treatments
    curr_row <- 1
    for(i in seq_along(pstage_per_treat_tot)[-length(seq_along(pstage_per_treat_tot))]) {
      writeData(wb, paste0(toupper(m),"-PStage&Trt"), names(pstage_per_treat_tot)[i], startCol = 1, startRow = curr_row)
      addStyle(wb, sheet = paste0(toupper(m),"-PStage&Trt"), createStyle(textDecoration = "bold", border = "TopBottomLeftRight", borderColour = "#999999", fontSize = 12, fontColour = "#cc0000"), rows = curr_row, cols = 1)
      writeData(wb, paste0(toupper(m),"-PStage&Trt"), pstage_per_treat_tot[[i]], startCol = 1, startRow = curr_row + 1, rowNames = TRUE, borders = "all", borderColour = "#999999",
                headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
      curr_row <- curr_row + nrow(pstage_per_treat_tot[[i]]) + 4
    }
    setColWidths(wb,sheet = paste0(toupper(m),"-PStage&Trt"),cols = 1:ncol(pstage_per_treat_tot[[1]]), widths = "auto")
    ...
    # Follow-up
    mergeCells(wb, paste0(toupper(m),"-FUP"), cols = 1, rows = 1:2)
    mergeCells(wb, paste0(toupper(m),"-FUP"), cols = 2, rows = 1:2)
    mergeCells(wb, paste0(toupper(m),"-FUP"), cols = 3:ncol(final_fup_df), rows = 1)
    mergeCells(wb, paste0(toupper(m),"-FUP"), cols = 1, rows = 3:(3+length(seq_year)-1))
    mergeCells(wb, paste0(toupper(m),"-FUP"), cols = 1, rows = (3+length(seq_year)):((3+length(seq_year))+length(seq_year)-1))
    mergeCells(wb, paste0(toupper(m),"-FUP"), cols = 1, rows = ((3+length(seq_year))+length(seq_year)):(((3+length(seq_year))+length(seq_year))+length(seq_year)-1))
    mergeCells(wb, paste0(toupper(m),"-FUP"), cols = 1, rows = (((3+length(seq_year))+length(seq_year))+length(seq_year)):((((3+length(seq_year))+length(seq_year))+length(seq_year))+length(seq_year)-1))
    writeData(wb,sheet = paste0(toupper(m),"-FUP"), final_fup_df, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
    setColWidths(wb,sheet = paste0(toupper(m),"-FUP"),cols = 1:ncol(final_fup_df), widths = "auto")
    
    # Lost at Follow-up
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 1, rows = 1:2)
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 2, rows = 1:2)
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 3, rows = 1:2)
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 4:ncol(lost_fup_df), rows = 1)
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 1, rows = 3:(3+2*length(seq_year)-1))
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 1, rows = (3+2*length(seq_year)):((3+2*length(seq_year))+2*length(seq_year)-1))
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 2, rows = 3:(3+length(seq_year)-1))
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 2, rows = (3+length(seq_year)):((3+length(seq_year))+length(seq_year)-1))
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 2, rows = ((3+length(seq_year))+length(seq_year)):(((3+length(seq_year))+length(seq_year))+length(seq_year)-1))
    mergeCells(wb, paste0(toupper(m),"-LostFUP"), cols = 2, rows = (((3+length(seq_year))+length(seq_year))+length(seq_year)):((((3+length(seq_year))+length(seq_year))+length(seq_year))+length(seq_year)-1))
    writeData(wb,sheet = paste0(toupper(m),"-LostFUP"), lost_fup_df, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
    setColWidths(wb,sheet = paste0(toupper(m),"-LostFUP"),cols = 1:ncol(lost_fup_df), widths = "auto")
    
    # Max number of repetitions
    writeData(wb,sheet = paste0(toupper(m),"-MaxRep"), maxRep, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
    setColWidths(wb,sheet = paste0(toupper(m),"-MaxRep"),cols = 1:ncol(maxRep), widths = "auto")
  }
  ...
  # Staging & Treatments ---------
  data_trt_complete <- redcap_data %>% subset(redcap_event_name == "non_repeatable_arm_1") %>%
    subset(e57_complete == 1 & f107_complete == 1) %>%
    mutate(treatment = case_when(
      # SOLO UN TRATTAMENTO
      f01_surgery %in% c(1,2) & !(f53_radio %in% c(1,2)) & !(f29_1_systrt %in% c(1,2)) & !(f29_2_systrt %in% c(1,2)) & !(f29_3_systrt %in% c(1,2)) & (min(f02_datesurg, f12_datenecksurg, na.rm = TRUE) <= (d01_diagdate + 90)) ~ "Only surgery",
  ...
  maxRep <- setNames(data.frame(matrix(ncol = 3, nrow = 0)), c("adverse_events", "imaging_available", "tumor_specimen_available"))
  for (i in colnames(maxRep)) {
    maxRep[1,i] <- ifelse(nrow(filter(redcap_data, redcap_repeat_instrument == i) > 0), max(filter(redcap_data, redcap_repeat_instrument == i)$redcap_repeat_instance), 0)
  }
  
  # add worksheet
  addWorksheet(wb, "Missing & Unknown")
  addWorksheet(wb, "Complete patients")
  addWorksheet(wb, "Clinical stage & Treatment")
  addWorksheet(wb, "Pathological stage & Treatment")
  addWorksheet(wb, "Year of diagnosis & Sites")
  addWorksheet(wb, "Core variables")
  addWorksheet(wb, "Follow-up")
  addWorksheet(wb, "Lost at Follow-up")
  addWorksheet(wb, "Max repetitions")
  ...
  # Complete patients
  mergeCells(wb, "Complete patients", cols = 1, rows = 2:12)
  mergeCells(wb, "Complete patients", cols = 1, rows = 13:14)
  writeData(wb,sheet = "Complete patients", case_complete, borders = "all", borderColour = "#999999",
            headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
  setColWidths(wb,sheet = "Complete patients",cols = 1:ncol(case_complete), widths = "auto")
  
  # Clinical stage & Treatment
  curr_row <- 1
  for(i in seq_along(cstage_per_treat_tot)[-length(seq_along(cstage_per_treat_tot))]) {
    writeData(wb, "Clinical stage & Treatment", names(cstage_per_treat_tot)[i], startCol = 1, startRow = curr_row)
    addStyle(wb, sheet = 'Clinical stage & Treatment', createStyle(textDecoration = "bold", border = "TopBottomLeftRight", borderColour = "#999999", fontSize = 12, fontColour = "#cc0000"), rows = curr_row, cols = 1)
    writeData(wb, "Clinical stage & Treatment", cstage_per_treat_tot[[i]], startCol = 1, startRow = curr_row + 1, rowNames = TRUE, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
    curr_row <- curr_row + nrow(cstage_per_treat_tot[[i]]) + 4
  }
  setColWidths(wb,sheet = "Clinical stage & Treatment",cols = 1:ncol(cstage_per_treat_tot[[1]]), widths = "auto")
  
  # Pathological stage & Treatment
  curr_row <- 1
  for(i in seq_along(pstage_per_treat_tot)[-length(seq_along(pstage_per_treat_tot))]) {
    writeData(wb, "Pathological stage & Treatment", names(pstage_per_treat_tot)[i], startCol = 1, startRow = curr_row)
    addStyle(wb, sheet = 'Pathological stage & Treatment', createStyle(textDecoration = "bold", border = "TopBottomLeftRight", borderColour = "#999999", fontSize = 12, fontColour = "#cc0000"), rows = curr_row, cols = 1)
    writeData(wb, "Pathological stage & Treatment", pstage_per_treat_tot[[i]], startCol = 1, startRow = curr_row + 1, rowNames = TRUE, borders = "all", borderColour = "#999999",
              headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
    curr_row <- curr_row + nrow(pstage_per_treat_tot[[i]]) + 4
  }
  setColWidths(wb,sheet = "Pathological stage & Treatment",cols = 1:ncol(pstage_per_treat_tot[[1]]), widths = "auto")
  ...
  # Follow-up
  mergeCells(wb, "Follow-up", cols = 1, rows = 1:2)
  mergeCells(wb, "Follow-up", cols = 2, rows = 1:2)
  mergeCells(wb, "Follow-up", cols = 3:ncol(final_fup_df), rows = 1)
  mergeCells(wb, "Follow-up", cols = 1, rows = 3:(3+length(seq_year)-1))
  mergeCells(wb, "Follow-up", cols = 1, rows = (3+length(seq_year)):((3+length(seq_year))+length(seq_year)-1))
  mergeCells(wb, "Follow-up", cols = 1, rows = ((3+length(seq_year))+length(seq_year)):(((3+length(seq_year))+length(seq_year))+length(seq_year)-1))
  mergeCells(wb, "Follow-up", cols = 1, rows = (((3+length(seq_year))+length(seq_year))+length(seq_year)):((((3+length(seq_year))+length(seq_year))+length(seq_year))+length(seq_year)-1))
  writeData(wb,sheet = "Follow-up", final_fup_df, borders = "all", borderColour = "#999999",
            headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
  setColWidths(wb,sheet = "Follow-up",cols = 1:ncol(final_fup_df), widths = "auto")
  
  # Lost at Follow-up
  mergeCells(wb, "Lost at Follow-up", cols = 1, rows = 1:2)
  mergeCells(wb, "Lost at Follow-up", cols = 2, rows = 1:2)
  mergeCells(wb, "Lost at Follow-up", cols = 3, rows = 1:2)
  mergeCells(wb, "Lost at Follow-up", cols = 4:ncol(lost_fup_df), rows = 1)
  mergeCells(wb, "Lost at Follow-up", cols = 1, rows = 3:(3+2*length(seq_year)-1))
  mergeCells(wb, "Lost at Follow-up", cols = 1, rows = (3+2*length(seq_year)):((3+2*length(seq_year))+2*length(seq_year)-1))
  mergeCells(wb, "Lost at Follow-up", cols = 2, rows = 3:(3+length(seq_year)-1))
  mergeCells(wb, "Lost at Follow-up", cols = 2, rows = (3+length(seq_year)):((3+length(seq_year))+length(seq_year)-1))
  mergeCells(wb, "Lost at Follow-up", cols = 2, rows = ((3+length(seq_year))+length(seq_year)):(((3+length(seq_year))+length(seq_year))+length(seq_year)-1))
  mergeCells(wb, "Lost at Follow-up", cols = 2, rows = (((3+length(seq_year))+length(seq_year))+length(seq_year)):((((3+length(seq_year))+length(seq_year))+length(seq_year))+length(seq_year)-1))
  writeData(wb,sheet = "Lost at Follow-up", lost_fup_df, borders = "all", borderColour = "#999999",
            headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
  setColWidths(wb,sheet = "Lost at Follow-up",cols = 1:ncol(lost_fup_df), widths = "auto")
  
  # Max number of repetitions
  writeData(wb,sheet = "Max repetitions", maxRep, borders = "all", borderColour = "#999999",
            headerStyle = createStyle(border = c("top", "bottom", "left", "right"), borderColour = "#999999", textDecoration = "bold"))
  setColWidths(wb,sheet = "Max repetitions",cols = 1:ncol(maxRep), widths = "auto")
}
