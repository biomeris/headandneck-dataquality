...
renderLogic <- function(br_logic_str){
  expr_str <- br_logic_str %>% gsub(' OR | or ', ' | ', .) %>%
    gsub(' AND | and ', ' & ', .) %>%
    gsub('=', '==', .) %>%
    gsub('\\[', '', .) %>%
    gsub('\\]', '', .) %>%
...
redcap_data[,c("a07_complete", "b24_complete","c28_complete", "d33_complete", "e57_complete", "f138_complete",
        grep("g[[:digit:]]+_119_complete", colnames(redcap_data), value = TRUE), "h06_complete", "i07_complete")] <- ps_ctrl[,c("a07_complete", "b24_complete","c28_complete", "d33_complete", "e57_complete", "f138_complete",
...
# metadata_tmp <- subset(metadata_tmp,!(field_name %in% c("d04_diagage_man", "g1_96_ottman", "g2_96_ottman", "g3_96_ottman", "g4_96_ottman", "g5_96_ottman","g6_96_ottman","g7_96_ottman","g8_96_ottman","g9_96_ottman","g10_96_ottman")))
# 
# for (i in 1:nrow(metadata_tmp)){
